...
WEB_HOST = "127.0.0.1"  # не менять: так админку не откроют из сети
...
def allowed_hosts() -> set:
    return {f"127.0.0.1:{WEB_PORT}", f"localhost:{WEB_PORT}", f"[::1]:{WEB_PORT}"}
...
async def dispatch(req: Req) -> Resp:
    ...
    if req.headers.get("host", "") not in allowed_hosts():
        raise HttpError(403, "Откройте панель по адресу http://127.0.0.1:%s" % WEB_PORT)
