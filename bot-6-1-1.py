...
def _load_dotenv(path):
    """Читает файл .env рядом с ботом (если он есть) — так секреты не попадают в GitHub."""
    try:
...
# Секреты хранятся НЕ в коде, а в переменных окружения хостинга или в файле .env
# (см. .env.example). Так их безопасно выкладывать на GitHub.
BOT_TOKEN = os.getenv("BOT_TOKEN", "")

# Вход в админку в боте: команда  /admin ПАРОЛЬ
# После первого входа бот запоминает вас, дальше хватает просто /admin.
ADMIN_PASSWORD = os.getenv("ADMIN_PASSWORD", "")

# Веб-админка (сайт). Включается, если задан WEB_PASSWORD.
# Порт хостинг обычно передаёт сам в переменной PORT.
...
# Канал, который выдаётся покупателю после покупки
BUYERS_CHANNEL_URL = "https://t.me/+aO0aEfw38GgwYWRi"
...
# Премиум-эмодзи в сообщении после покупки (можно поменять местами по вкусу)
BUY_EMOJI_OK = "5258508428212445001"      # заголовок «Покупка успешна»
BUY_EMOJI_PRODUCT = "5399986364634641475"  # название товара
BUY_EMOJI_TERM = "5775896410780079073"     # срок
BUY_EMOJI_PAID = "5231449120635370684"     # списано (то же, что «Цена» в карточке)
BUY_EMOJI_KEY = "5258514780469075716"      # ключ
BUY_EMOJI_RATE = "5280640845660330048"     # просьба оценить / канал
...
def create_review(order_id, uid, username, rating):
    """Сохраняет оценку. Возвращает id отзыва или None, если на этот заказ отзыв уже есть
    (или заказ не принадлежит пользователю)."""
    o = q1("SELECT product_name FROM orders WHERE id=? AND user_id=?", order_id, uid)
    if not o:
        return None
    cur = run(
        "INSERT OR IGNORE INTO reviews(order_id, user_id, username, product_name, rating) "
        "VALUES(?, ?, ?, ?, ?)",
        order_id, uid, username, o["product_name"], rating,
    )
    return cur.lastrowid if cur.rowcount else None


def finish_review(review_id, uid, text):
    """Сохраняет текст отзыва (или None) и закрывает его. True — если закрыт впервые
    (нужно уведомить админов), False — если уже был закрыт раньше."""
    cur = run(
        "UPDATE reviews SET text=?, notified=1 WHERE id=? AND user_id=? AND notified=0",
        text, review_id, uid,
    )
...
def channel_button():
    return ib("Наш канал", url=BUYERS_CHANNEL_URL, style=GREEN)


def purchase_kb(order_id: int) -> InlineKeyboardMarkup:
    """После покупки: ряд из 5 звёзд для оценки, канал и главное меню."""
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [ib(f"{n} ⭐", f"rv:{order_id}:{n}") for n in range(1, 6)],
            [channel_button()],
...
def after_rating_kb() -> InlineKeyboardMarkup:
    """Клавиатура под сообщением с ключом после того, как оценка поставлена."""
    return kb(channel_button(), ib("Главное меню", "go:main"))


def purchase_text(p, code) -> str:
    return (
        f"{em(BUY_EMOJI_OK, '✅')} Покупка успешна\n\n"
        f"{em(BUY_EMOJI_PRODUCT, '📦')} {esc(p['title'])} · {esc(p['name'])}\n"
        f"{em(BUY_EMOJI_TERM, '🕓')} Срок: {days_text(p['days'])}\n"
        f"{em(BUY_EMOJI_PAID, '💸')} Списано: {p['price']} ₽\n\n"
        f"{em(BUY_EMOJI_KEY, '🔑')} Ваш ключ:\n<code>{esc(code)}</code>\n\n"
        f"{em(BUY_EMOJI_RATE, '⭐')} Присоединяйтесь к нашему каналу — кнопка ниже.\n"
        "Оцените покупку от 1 до 5 звёзд, а потом напишите пару слов в отзыве."
    )
...
async def pay_balance(cb: CallbackQuery):
    uid = cb.from_user.id
    ensure_user(uid)
    status, p, code, order_id = purchase(uid, int(cb.data.split(":")[2]))
    ...
    await show(cb.message, purchase_text(p, code), purchase_kb(order_id), None)


# ── отзывы после покупки
async def notify_admins_review(bot: Bot, review_id: int):
    """Отправляет готовый отзыв всем админам."""
    r = q1("SELECT * FROM reviews WHERE id=?", review_id)
    if not r:
        return
    who = f"@{esc(r['username'])}" if r["username"] else "без username"
    text = (
        f"⭐ Новый отзыв #{r['id']}\n\n"
        f"Оценка: {stars(r['rating'])} ({r['rating']}/5)\n"
        f"Товар: {esc(r['product_name'])}\n"
        f"Покупатель: {who} · ID <code>{r['user_id']}</code>\n\n"
        + (esc(r["text"]) if r["text"] else "— без текста —")
    )
    markup = kb(ib("⭐ Все отзывы", "a:rv"))
...
async def review_rate(cb: CallbackQuery, state: FSMContext):
    _, order_id, rating = cb.data.split(":")
    uid = cb.from_user.id
    rid = create_review(int(order_id), uid, cb.from_user.username, int(rating))
    if rid is None:
        await cb.answer("Отзыв на эту покупку уже оставлен.", show_alert=True)
        try:
    ...
    try:  # убираем звёзды из сообщения с ключом, ключ остаётся
        await cb.message.edit_reply_markup(reply_markup=after_rating_kb())
    except TelegramBadRequest:
        pass
    ...
    await cb.message.answer(
        f"{em(BUY_EMOJI_RATE, '⭐')} Ваша оценка: {stars(int(rating))}\n\n"
        "Теперь напишите отзыв одним сообщением — нам важно ваше мнение.",
        reply_markup=kb(ib("Пропустить", f"rv:skip:{rid}")),
    )
...
async def review_text(msg: Message, state: FSMContext, bot: Bot):
    ...
    if rid and finish_review(rid, msg.from_user.id, msg.text.strip()[:1000]):
        await msg.answer(
            f"{em(BUY_EMOJI_OK, '✅')} Спасибо за отзыв! Он уже у администратора.",
            reply_markup=kb(channel_button(), ib("Главное меню", "go:main")),
        )
        await notify_admins_review(bot, rid)
    else:
        await msg.answer("Отзыв уже отправлен.", reply_markup=kb(ib("Главное меню", "go:main")))


@user_r.message(ReviewState.text)
async def review_text_wrong(msg: Message):
    await msg.answer("Напишите отзыв текстом или нажмите «Пропустить».")


@user_r.callback_query(F.data.regexp(r"^rv:skip:\d+$"))
async def review_skip(cb: CallbackQuery, state: FSMContext, bot: Bot):
    rid = int(cb.data.split(":")[2])
    await state.clear()
    if finish_review(rid, cb.from_user.id, None):
        await cb.answer("Спасибо за оценку!")
        await show(
            cb.message,
            f"{em(BUY_EMOJI_OK, '✅')} Спасибо за оценку!",
            kb(channel_button(), ib("Главное меню", "go:main")),
        )
        await notify_admins_review(bot, rid)
    else:
        await cb.answer("Отзыв уже отправлен.", show_alert=True)
...
def screen_admin():
    ...
    reviews = scalar("SELECT COUNT(*) FROM reviews") or 0
    text = f"🛠 Админ-панель\n\nПользователей: {users}\nЗаказов: {orders}\nОтзывов: {reviews}"
    markup = kb(
        ib("📊 Оборот", "a:stats", style=GREEN),
        ib("⭐ Отзывы", "a:rv", style=GREEN),
        ib("📦 Товары", "a:prods"),
        ib("🖼 Баннеры", "a:bn"),
        ib("📣 Объявление", "a:bc"),
        ib("🎁 Промокоды", "a:pr"),
...
def screen_reviews(offset=0):
    total = scalar("SELECT COUNT(*) FROM reviews") or 0
    if not total:
        return "⭐ Отзывов пока нет.", kb(ib("Назад", "a:menu")), None
    offset = max(0, min(offset, (total - 1) // REVIEWS_PAGE * REVIEWS_PAGE))
    avg = scalar("SELECT AVG(rating) FROM reviews") or 0
    dist = {r["rating"]: r["c"] for r in q("SELECT rating, COUNT(*) AS c FROM reviews GROUP BY rating")}
    dist_line = " · ".join(f"{n}★ {dist.get(n, 0)}" for n in (5, 4, 3, 2, 1))
    rows = q(
        "SELECT *, strftime('%d.%m %H:%M', created, ?) AS d FROM reviews "
        "ORDER BY id DESC LIMIT ? OFFSET ?",
        _off(), REVIEWS_PAGE, offset,
    )
    blocks = []
    for r in rows:
        who = f"@{esc(r['username'])}" if r["username"] else f"ID {r['user_id']}"
        body = r["text"] or "— без текста —"
        if len(body) > 200:
            body = body[:200] + "…"
        blocks.append(
            f"#{r['id']} {stars(r['rating'])} · {esc(r['product_name'])}\n"
            f"{who} · {r['d']}\n{esc(body)}"
        )
    text = (
        f"⭐ Отзывы\n\nСредняя оценка: {avg:.2f} · всего: {total}\n{dist_line}\n\n"
        + "\n\n".join(blocks)
    )
    nav = []
    if offset > 0:
        nav.append(ib("◀️ Новее", f"a:rv:{max(0, offset - REVIEWS_PAGE)}"))
    if offset + REVIEWS_PAGE < total:
        nav.append(ib("Старше ▶️", f"a:rv:{offset + REVIEWS_PAGE}"))
    rows_kb = ([nav] if nav else []) + [[ib("Назад", "a:menu")]]
    return text, InlineKeyboardMarkup(inline_keyboard=rows_kb), None
...
async def main():
    ...
    if not BOT_TOKEN:
        raise SystemExit(
            "Не задан BOT_TOKEN. Укажите его в переменных окружения хостинга "
    ...
    if WEB_PASSWORD:
        try:
            from webadmin import start_web

            web_runner = await start_web(sys.modules[__name__], bot, WEB_PASSWORD, WEB_PORT)
        except Exception:
            log.exception("Веб-админка не запустилась — бот продолжит работать без неё")
    else:
        log.info("WEB_PASSWORD не задан — веб-админка выключена")
