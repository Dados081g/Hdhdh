...
CANCEL_KB = kb(ib("Отмена", "a:cancel"))
...
def screen_card(pid):
    ...
    return card_text(p), kb(*buttons), "devices"
...
def screen_pay(pid, uid):
    ...
    return card_text(p), markup, "devices"
...
def screen_profile(uid):
    ...
    markup = kb(
        ...
        ib("Отзывы", url=REVIEWS_URL, emoji="5280472706280622601"),
        ib("Поддержка", "go:support", emoji="5904630315946611415"),
...
def screen_policy():
    ...
    markup = kb(
        ...
        ib("Пользовательское соглашение", url=TERMS_URL),
        ib("Назад", "go:profile"),
    )
    return text, markup, "profile"
...
def screen_support():
    return SUPPORT_TEXT, kb(ib("Назад", "go:profile")), "profile"
...
admin_r.message.filter(F.from_user.id.in_(ADMIN_IDS))
admin_r.callback_query.filter(F.from_user.id.in_(ADMIN_IDS))
...
async def pay_balance(cb: CallbackQuery):
    uid = cb.from_user.id
    ensure_user(uid)
    status, p, code = purchase(uid, int(cb.data.split(":")[2]))
    ...
    text = (
        "✅ Покупка успешна\n\n"
        f"{esc(p['title'])} · {esc(p['name'])}\n"
        f"Срок: {days_text(p['days'])}\n"
        f"Списано: {p['price']} ₽\n\n"
        f"Ваш ключ:\n<code>{esc(code)}</code>"
    )
    await show(cb.message, text, kb(ib("Главное меню", "go:main")), None)
...
async def a_bn_save(msg: Message, state: FSMContext):
    ...
    await msg.answer_photo(
        file_id,
        caption=f"✅ Баннер «{BANNER_SLOTS[slot]}» установлен.",
        reply_markup=kb(ib("К баннерам", "a:bn")),
    )
