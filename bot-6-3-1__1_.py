...
# ─── Оплата по СБП через Platega (app.platega.io) ───
# MerchantId и API-ключ выдаются в личном кабинете Platega (раздел «Настройки»).
# В личном кабинете Platega, в разделе Callback URL, укажите:
#   http://<этот сервер>:<WEB_PORT>/webhook/platega   (например, http://85.193.87.122:34795/webhook/platega)
# Работает ТОЛЬКО если включена веб-админка (WEB_ENABLED=1) — она же принимает уведомления об оплате.
PLATEGA_MERCHANT_ID = os.getenv("PLATEGA_MERCHANT_ID", "c03c9e38-8ac4-40a5-996e-4296117f896d")
PLATEGA_SECRET = os.getenv("PLATEGA_SECRET", "K4lpOgafIuBc6mTwXxDhBaJP0a3lezvatnkHiZ4PzUrOhdn3MCy7kCQT29FE1NFYstO99rTEHLDsKavMOVmwuHTkhHkjjBM1tr9x")
PLATEGA_BASE_URL = "https://app.platega.io"
PLATEGA_PAYMENT_METHOD = 2  # 2 = СБП (QR-код), см. документацию Platega
PLATEGA_RETURN_URL = f"http://{WEB_PUBLIC_HOST or '127.0.0.1'}:{WEB_PORT}/pay/ok"
PLATEGA_FAILED_URL = f"http://{WEB_PUBLIC_HOST or '127.0.0.1'}:{WEB_PORT}/pay/fail"
...
# ═══════════════════ ОПЛАТА ПО СБП (Platega) ═══════════════════
def _platega_call(method: str, path: str, body: dict | None = None):
    """Синхронный HTTP-запрос к Platega (выполняется в отдельном потоке)."""
    url = PLATEGA_BASE_URL.rstrip("/") + path
    data = json.dumps(body).encode() if body is not None else None
    req = urllib.request.Request(url, data=data, method=method)
    req.add_header("X-MerchantId", PLATEGA_MERCHANT_ID)
    req.add_header("X-Secret", PLATEGA_SECRET)
    if data is not None:
    ...
    try:
        ...
        log.error("Platega %s %s: %s", method, path, e)
        return 0, {"error": str(e)}


async def platega_call(method: str, path: str, body: dict | None = None):
    return await asyncio.get_running_loop().run_in_executor(None, _platega_call, method, path, body)


async def platega_create_transaction(amount: int, description: str, payload: str = ""):
    """Создаёт платёж на оплату по СБП. Возвращает (успех, данные_или_текст_ошибки)."""
    if not (PLATEGA_MERCHANT_ID and PLATEGA_SECRET):
        return False, "Оплата по СБП не настроена (нет PLATEGA_MERCHANT_ID / PLATEGA_SECRET)."
    status, data = await platega_call(
        "POST",
        "/transaction/process",
        {
    ...
    if status == 200 and data.get("transactionId") and data.get("redirect"):
        return True, data
    log.error("Platega create_transaction: %s %s", status, data)
    return False, "Не удалось создать платёж. Попробуйте ещё раз чуть позже."


async def platega_get_status(transaction_id: str):
    status, data = await platega_call("GET", f"/transaction/{transaction_id}")
...
def fulfill_stock_order(uid, product_id):
    """Выдача ключа напрямую, без списания с баланса (после прямой оплаты по СБП).
    Возвращает (товар, ключ_или_None, id_заказа_или_None)."""
    p = q1("SELECT * FROM products WHERE id=?", product_id)
    if not p:
...
def screen_profile(uid):
    ...
    markup = kb(
        ib("Пополнить баланс (СБП)", "topup:ask", style=GREEN),
...
async def offer_sbp_payment(uid, amount, description, product_id, back_button):
    """Создаёт платёж в Platega и готовит (текст, клавиатуру) для показа пользователю.
    Возвращает ((текст, клавиатура), None) при успехе или (None, текст_ошибки) при неудаче."""
    ok, data = await platega_create_transaction(amount, description, payload=str(uid))
    if not ok:
        return None, data  # data — это текст ошибки
    run(
        "INSERT INTO payments(id, user_id, product_id, amount, status) VALUES(?,?,?,?, 'PENDING')",
        data["transactionId"], uid, product_id, amount,
    )
    markup = kb(
        ib("Оплатить по СБП", url=data["redirect"], style=GREEN),
        ib("Я оплатил(а)", f"pay:check:{data['transactionId']}", style=BLUE),
        back_button,
    )
    text = (
        f"{esc(description)}\n{LINE}\n"
        f"Сумма к оплате: {amount} ₽\n\n"
        "Нажмите «Оплатить по СБП», завершите оплату и вернитесь сюда — обычно зачисление "
        "происходит автоматически в течение минуты. Если этого не произошло, нажмите «Я оплатил(а)»."
    )
...
async def pay_sbp(cb: CallbackQuery):
    ...
    if not p:
        ...
        return
    if not q1("SELECT 1 FROM stock WHERE product_id=? AND used=0 LIMIT 1", pid):
        await cb.answer("Товар временно закончился. Попробуйте позже.", show_alert=True)
        return
    await cb.answer("Формируем ссылку на оплату…")
    result, err = await offer_sbp_payment(
        uid, p["price"], f"{p['title']} · {p['name']}", pid, ib("Назад", f"buy:{pid}")
    )
    if err:
        await show(cb.message, err, kb(ib("Назад", f"buy:{pid}")), None)
        return
    text, markup = result
    await show(cb.message, text, markup, product_slot(pid))


@user_r.callback_query(F.data == "topup:ask")
async def topup_ask(cb: CallbackQuery, state: FSMContext):
    if not (PLATEGA_MERCHANT_ID and PLATEGA_SECRET):
    ...
    await show(
        cb.message,
        "Введите сумму пополнения в рублях (от 10 до 100 000):",
        kb(ib("Отмена", "go:profile")),
        "profile",
    )


@user_r.message(TopUp.amount, F.text)
async def topup_amount(msg: Message, state: FSMContext):
    await state.clear()
    uid = msg.from_user.id
    ensure_user(uid)
    amount = pint(msg.text, 10, 100_000)
    if amount is None:
        await msg.answer(
            "Сумма должна быть целым числом от 10 до 100 000 ₽. Попробуйте ещё раз через «Профиль».",
            reply_markup=kb(ib("Профиль", "go:profile")),
        )
        return
    result, err = await offer_sbp_payment(uid, amount, "Пополнение баланса", None, ib("Профиль", "go:profile"))
    if err:
        await msg.answer(esc(err), reply_markup=kb(ib("Профиль", "go:profile")))
        return
    text, markup = result
...
async def deliver_sbp_payment(bot, row) -> bool:
    """Обрабатывает подтверждённый платёж Platega (row из таблицы payments, ещё в статусе PENDING):
    выдаёт ключ или зачисляет баланс и уведомляет пользователя. Вызывается и из webhook, и при
    ручной проверке — атомарная отметка статуса защищает от повторной обработки одного платежа."""
    cur = db.execute("UPDATE payments SET status='DONE' WHERE id=? AND status='PENDING'", (row["id"],))
    db.commit()
    if cur.rowcount == 0:
        return False  # уже обработан
    uid, pid, amount = row["user_id"], row["product_id"], row["amount"]
    ensure_user(uid)
    if pid is None:  # это было пополнение баланса
        _, _, bal = change_balance(uid, amount)
        try:
            await bot.send_message(
                uid, f"💰 Оплата получена. На баланс зачислено {amount} ₽.\nТекущий баланс: {bal} ₽."
            )
        except Exception:
            log.exception("Не удалось уведомить пользователя %s о пополнении", uid)
        return True
    p, code, oid = fulfill_stock_order(uid, pid)
    if not p or code is None:
        # Деньги не теряем: если товар пропал/закончился, зачисляем сумму на баланс.
        change_balance(uid, amount)
        try:
            await bot.send_message(
                uid,
                "Оплата получена, но товар закончился или был удалён. "
                f"{amount} ₽ зачислено на баланс — можно выбрать другой тариф.",
            )
        except Exception:
            log.exception("Не удалось уведомить пользователя %s", uid)
        return True
    try:
        await bot.send_message(uid, success_text(p, code), reply_markup=success_kb())
        await bot.send_message(uid, REVIEW_ASK_TEXT, reply_markup=stars_kb(oid))
    except Exception:
        log.exception("Не удалось отправить ключ пользователю %s", uid)
    return True
...
@user_r.callback_query(F.data.startswith("pay:check:"))
async def pay_check(cb: CallbackQuery):
    txid = cb.data.split(":", 2)[2]
    row = q1("SELECT * FROM payments WHERE id=?", txid)
    if not row:
        await cb.answer("Платёж не найден.", show_alert=True)
        return
    if row["user_id"] != cb.from_user.id:
        await cb.answer("Это не ваш платёж.", show_alert=True)
        return
    if row["status"] != "PENDING":
        await cb.answer("Этот платёж уже обработан.", show_alert=True)
        return
    data = await platega_get_status(txid)
    if not data:
        await cb.answer("Не удалось проверить статус, попробуйте чуть позже.", show_alert=True)
        return
    status = data.get("status")
    if status == "CONFIRMED":
        await deliver_sbp_payment(cb.bot, row)
        await cb.answer("Оплата подтверждена!")
        await show(
            cb.message,
            "✅ Оплата подтверждена — детали отправлены отдельным сообщением.",
            kb(ib("Главное меню", "go:main")),
            None,
        )
    elif status in ("CANCELED", "CHARGEBACKED"):
        run("UPDATE payments SET status=? WHERE id=? AND status='PENDING'", status, txid)
        await cb.answer("Платёж отменён или не прошёл.", show_alert=True)
    else:
        await cb.answer("Платёж пока в обработке. Подождите немного и попробуйте снова.", show_alert=True)
...
PAY_PAGE = (
    ...
    '<body style="font:16px system-ui;background:#0f0d12;color:#eee;padding:40px;text-align:center">'
    "<h2>{title}</h2><p>{text}</p></body>"
)


async def w_pay_ok(req):
    return Resp(PAY_PAGE.format(
        title="Оплата получена ✅",
        text="Возвращайтесь в Telegram — бот подтвердит платёж и пришлёт результат в течение минуты.",
    ))


async def w_pay_fail(req):
    return Resp(PAY_PAGE.format(
        title="Оплата не завершена ❌",
        text="Вернитесь в Telegram и попробуйте оплатить ещё раз.",
    ))


async def w_webhook_platega(req):
    """Приём callback от Platega об изменении статуса транзакции."""
    if not (PLATEGA_MERCHANT_ID and PLATEGA_SECRET):
        raise HttpError(404)
    if (
        req.headers.get("x-merchantid") != PLATEGA_MERCHANT_ID
        or req.headers.get("x-secret") != PLATEGA_SECRET
    ):
        raise HttpError(401)
    try:
        ...
        raise HttpError(400)
    txid = str(payload.get("id") or "")
    status = payload.get("status")
    if txid and status in ("CONFIRMED", "CANCELED", "CHARGEBACKED"):
        row = q1("SELECT * FROM payments WHERE id=?", txid)
        if row and row["status"] == "PENDING":
            if status == "CONFIRMED":
                await deliver_sbp_payment(WEB["bot"], row)
            else:
                run("UPDATE payments SET status=? WHERE id=? AND status='PENDING'", status, txid)
    return Resp(b"ok", ctype="text/plain; charset=utf-8")
...
ROUTES = [
    ...
    ("GET", r"/pay/ok", w_pay_ok, True),
    ("GET", r"/pay/fail", w_pay_fail, True),
    ("POST", r"/webhook/platega", w_webhook_platega, True),
